/*
 * Created on 2025-11-07 ( 16:16:34 )
 * Generated by Telosys ( https://www.telosys.org/ ) version 4.2.0
 */
package com.organizame.reportes.persistence.repositories;

import org.springframework.data.repository.PagingAndSortingRepository;

import com.organizame.reportes.persistence.entities.VhcMarcaarchivo;

/**
 * Spring Data JPA repository for entity "VhcMarcaarchivo" <br> 
 * 
 * This repository extends PagingAndSortingRepository interface <br>
 * so it provides by default all the basic CRUD operations : <br>
 *   findById, findAll, save, delete, etc <br> 
 * with pagination and sorting : <br>
 *   findAll(Pageable), findAll(Sort)<br>
 * 
 * This repository can be extended by adding specific "finders" methods<br>
 * To do so, see the "predicates conventions" for "derived query methods" in Spring Data documentation
 * 
 * @author Telosys
 *
 */
public interface VhcMarcaarchivoRepository extends PagingAndSortingRepository<VhcMarcaarchivo, byte[]> {

	// Insert specific finders here 

	//List<VhcMarcaarchivo> findByXxx(String xxx);

	//List<VhcMarcaarchivo> findByXxxStartingWith(String xxx);

	//List<VhcMarcaarchivo> findByXxxContaining(String xxx);

	//List<VhcMarcaarchivo> findByYyy(BigDecimal yyy);

	//List<VhcMarcaarchivo> findByXxxContainingAndYyy(String xxx, BigDecimal yyy);
}
