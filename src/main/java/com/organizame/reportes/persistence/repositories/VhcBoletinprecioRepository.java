...
public interface VhcBoletinprecioRepository extends JpaRepository<VhcBoletinprecio, byte[]> {
    ...
    @Query("select v from VhcBoletinprecio v where v.fechainicio <= :fecha and v.aniomyarchivo like :anio")
    List<VhcBoletinprecio> lastedBoletin(@Param("fecha")LocalDateTime fechainicioStart, @Param("anio") String year);
