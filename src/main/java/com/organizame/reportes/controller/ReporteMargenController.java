...
import com.organizame.reportes.service.ReporteMargenUtilidad;
...
@Slf4j
@RestController
@RequestMapping("/reporte/margen")
public class ReporteMargenController {

    private final ReporteMargenUtilidad margenUtilidad;


    @Autowired
    public ReporteMargenController(ReporteMargenUtilidad margenUtilidad){
        this.margenUtilidad = margenUtilidad;
    }


    @Operation(summary = "regresa reporte Margen de utilidad")
    @ApiResponses(value = {
    ...
    @PostMapping("")
    @ResponseBody
    public ResponseEntity<?> postReporteRanking(@RequestBody RequestMargen request){
    ...
    private ResponseEntity<?> reporteMargen(RequestMargen request){
        try {
            ByteArrayInputStream resultado = margenUtilidad.CrearExcelRanking(request);
            var archivo = margenUtilidad.getNombreArchivo();
